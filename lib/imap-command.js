...
class IMAPCommand {
  ...
  append(command, callback) {
    ...
    if (command.literal) {
      ...
      if (
        isNaN(command.expecting) ||
        isNaN(command.expecting) < 0 ||
        command.expecting > Number.MAX_SAFE_INTEGER
      ) {
        errors.notifyConnection(
          this.connection,
          new Error('Invalid literal size'),
          {
            command: {
    ...
    this.append(command, err => {
      ...
      if (
        /^(AUTHENTICATE|LOGIN)/.test(this.command) &&
        Array.isArray(this.parsed.attributes)
      ) {
      ...
      this.validateCommand(this.parsed, handler, err => {
        if (err) {
          ...
          errors.notifyConnection(this.connection, err, {
            payload: payload
              ? payload.length < 256
                ? payload
                : payload.toString().substr(0, 150) + '...'
              : false
          });
          this.connection.send(
            this.tag + ' ' + (err.response || 'BAD') + ' ' + err.message
          );
          return next(err);
        }
        ...
        if (typeof handler.handler === 'function') {
          ...
          this.connection.send(
            this.tag + ' NO Not implemented: ' + this.command
          );
          return next();
        }
      });
  ...
  sendNotifications(handler, callback) {
    if (
      this.connection.state !== 'Selected' ||
  ...
  validateCommand(parsed, handler, callback) {
    ...
    if (
      handler.state &&
      [].concat(handler.state || []).indexOf(this.connection.state) < 0
    ) {
      return callback(
        new Error(parsed.command.toUpperCase() + ' not allowed now')
      );
    }
