            ...
            if (params[1]) {
              ...
              if (params[3]) {
                socketOptions.remotePort =
                  Number(params[3].trim()) || socketOptions.remotePort;
              }
            }
    ...
    if (typeof ctxMap.get === 'function') {
      ctxMap.forEach((ctx, servername) => {
        this.secureContext.set(
          ...
          tls.createSecureContext(tlsOptions(ctx))
        );
      });
    } else {
      Object.keys(ctxMap).forEach(servername => {
        this.secureContext.set(
          this._normalizeHostname(servername),
          tls.createSecureContext(tlsOptions(ctxMap[servername]))
        );
      });
    }
    ...
    if (this.options.secure) {
      ...
      if (typeof this.options.SNICallback !== 'function') {
        ...
        this.options.SNICallback = (servername, cb) => {
          cb(
            null,
            this.secureContext.get(this._normalizeHostname(servername)) ||
              this.secureContext.get('*')
          );
        };
      }
    }
