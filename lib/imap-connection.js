...
class IMAPConnection extends EventEmitter {
  ...
  init() {
    ...
    let sendGreeting = () => {
      ...
      this.send(
        '* OK ' +
          ((this._server.options.id && this._server.options.id.name) ||
            packageInfo.name) +
    ...
    let reverseCb = (err, hostnames) => {
      ...
      this.session.clientHostname = this.clientHostname =
        clientHostname || '[' + this.remoteAddress + ']';
  ...
  send(payload, callback) {
    if (this._socket && this._socket.writable) {
      this[!this.compression ? '_socket' : '_deflate'].write(
        payload + '\r\n',
        'binary',
        callback
      );
  ...
  _setListeners() {
    ...
    this._socket.setTimeout(
      this._server.options.socketTimeout || SOCKET_TIMEOUT,
  ...
  _onError(err) {
    ...
    if (
      ['ECONNRESET', 'EPIPE', 'ETIMEDOUT', 'EHOSTUNREACH'].includes(err.code)
    ) {
    ...
    if (
      err &&
      /SSL[23]*_GET_CLIENT_HELLO|ssl[23]*_read_bytes|ssl_bytes_to_cipher_list/i.test(
        err.message
      )
    ) {
  ...
  setupNotificationListener() {
    let conn = this;
    let isSelected = mailbox =>
      mailbox &&
      conn.selected &&
      conn.selected.mailbox &&
      conn.selected.mailbox.toString() === mailbox.toString();
    ...
    this._listenerData = {
      ...
      callback(message) {
        ...
        conn._server.notifier.getUpdates(
          selectedMailbox,
          conn.selected.modifyIndex,
          (err, updates) => {
            ...
            if (
              updates[updates.length - 1].modseq > conn.selected.modifyIndex
            ) {
              conn.selected.modifyIndex = updates[updates.length - 1].modseq;
            }

            // append received notifications to the list
            conn.selected.notifications = conn.selected.notifications.concat(
              updates
            );
            if (conn.idling) {
    ...
    this._server.notifier.addListener(
      this.session,
  ...
  clearNotificationListener() {
    ...
    this._server.notifier.removeListener(
      this.session,
      this._listenerData.callback
    );
  ...
  emitNotifications() {
    ...
    for (let i = this.selected.notifications.length - 1; i >= 0; i--) {
      ...
      if (update.command === 'FETCH') {
        ...
        if (
          fetches.has(update.uid) ||
          added.has(update.uid) ||
          removed.has(update.uid)
        ) {
      ...
      if ('query' in data) {
        ...
        data.query.forEach((item, i) => {
          ...
          if (['flags', 'modseq'].indexOf(item.item) >= 0) {
            ...
            Object.prototype.toString.call(data.values[i]) === '[object Date]'
          ) {
            response.attributes[1].push({
              type: 'ATOM',
