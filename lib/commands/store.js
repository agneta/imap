...
module.exports = {
  ...
  handler(command, callback) {
    ...
    let action = (
      (command.attributes[pos + 1] && command.attributes[pos + 1].value) ||
      ''
    )
      .toString()
      .toUpperCase();
    ...
    let extensions = !pos
      ? []
      : [].concat(command.attributes[pos] || []).map(val => val && val.value);

    if (extensions.length) {
      if (
        extensions.length !== 2 ||
        (extensions[0] || '').toString().toUpperCase() !== 'UNCHANGEDSINCE' ||
        isNaN(extensions[1])
      ) {
    ...
    this._server.onStore(
      ...
      (err, success, modified) => {
        ...
        if (
          !this._server.options.allowStoreExpunged &&
          success === true &&
          !silent &&
          messages.length
        ) {
          for (let i = this.selected.notifications.length - 1; i >= 0; i--) {
            if (
              this.selected.notifications[i].command === 'EXPUNGE' &&
              messages.includes(this.selected.notifications[i].uid)
            ) {
