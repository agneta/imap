...
module.exports = {
  ...
  handler(command, callback) {
    ...
    let isUid =
      (command.command || '').toString().toUpperCase() === 'UID FETCH'
        ? true
        : false;
    ...
    let messages = imapTools.getMessageRange(
      this.selected.uidList,
      range,
      isUid
    );
    ...
    let extensions = []
      .concat(command.attributes[2] || [])
      .map(val => val && val.value);

    if (extensions.length) {
      if (
        extensions.length !== 2 ||
        (extensions[0] || '').toString().toUpperCase() !== 'CHANGEDSINCE' ||
        isNaN(extensions[1])
      ) {
    ...
    if (
      command.attributes[1].type === 'ATOM' &&
      macros.has(command.attributes[1].value.toUpperCase())
    ) {
    ...
    for (i = 0, len = params.length; i < len; i++) {
      ...
      if (!param || (typeof param !== 'string' && param.type !== 'ATOM')) {
        return callback(
          new Error('Invalid message data item name for ' + command.command)
        );
      }
      ...
      if (
        [
          ...
          'RFC822.TEXT',
          'BODYSTRUCTURE'
        ].indexOf(param.value.toUpperCase()) >= 0
      ) {
        metadataOnly = false;
      }
    }
    ...
    for (i = 0, len = params.length; i < len; i++) {
      ...
      if (param.section) {
        if (!param.section.length) {
          ...
          /*
                    ...
                    */
          if (
            /^HEADER.FIELDS(\.NOT)?$/i.test(item.type) &&
            Array.isArray(param.section[1])
          ) {
      ...
      if (
        ['RFC822', 'RFC822.HEADER', 'RFC822.TEXT'].indexOf(
      ...
      if (
        !imapTools.fetchSchema.hasOwnProperty(item.item) ||
        !checkSchema(imapTools.fetchSchema[item.item], item)
      ) {
        return callback(null, {
          response: 'BAD',
          message:
            'Invalid message data item ' +
            item.query +
            ' for ' +
            command.command
        });
